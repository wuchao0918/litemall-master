	...
	<view>
		...
		<view class="footer">
			...
			<view class="btn">
				...
				<view class="buy" @tap="buy">借阅</view>
			</view>
		</view>
		<!-- share弹窗 -->
		...
		<view class="popup spec" :class="specClass" @touchmove.stop.prevent="discard" @tap="hideSpec">
			...
			<view class="layer" @tap.stop="discard">
				<view class="content">
					<view class="title">点击确认后完成借阅</view>
					<view class="sp">
					</view>
				</view>
				<view class="btn">
					<view class="button" @tap="hideSpec">确认</view>
				</view>
			</view>
		</view>
		<!-- 商品主图轮播 -->
...
<script>
	...
	export default {
		...
		onLoad(option) {
			...
			console.log("book_id11==" + option.book_id); //打印出上个页面传递的参数。
			util.request(api.GoodsDetail, {
				id: option.book_id
			}).then(function(res) {
				console.log(res.data);
				//uni.setStorageSync("thisbookinfo",res.data)
				this.swiperList=res.data.info.gallery;
				uni.setStorage({
					key: 'thisbookinfo',
					data: {
						swiperList: res.data.info.gallery

						
					}
				});
				//this.swiperList=res.data.info.gallery;
			//	consle.toLocaleString(res.data.info.gallery);
           console.log("book_id==onLoad"); //打印出上个页面传递的参数。
		...
		onReady(option) {
			...
			console.log("book_id==onReady"); //打印出上个页面传递的参数。
			console.log("book_id11222==" + option.book_id);
			console.log(this.swiperList)
			 uni.getStorage({
			 	key:'thisbookinfo',
				success: (res)=>{
